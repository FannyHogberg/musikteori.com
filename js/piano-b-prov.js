class PianoBTest {
    ...
    generatePiano(callback) {
        ...
        // Generate black keys with flat notation (since this is b-förtecken test)
        requestAnimationFrame(() => {
            const whiteKeyWidth = whiteKeys[0].element.offsetWidth;
            const gap = 2;

            // Mapping from base note to flat notation
            const flatMapping = {
                'C': 'Db',
                'D': 'Eb',
                'F': 'Gb',
            ...
            whiteKeys.forEach((keyData) => {
                if (['C', 'D', 'F', 'G', 'A'].includes(keyData.note)) {
                    ...
                    blackLabel.textContent = flatNote;
